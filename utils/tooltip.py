...
class Binding:
    ...
    def unbind(self) -> None:
        self._widget.unbind(self._name)
...
class ToolTip(tk.Toplevel):
    ...
    def _init_bindings(self) -> list[Binding]:
        ...
        if self.follow:
            if not self.persist:
                raise ValueError("'persist' argument must be True if 'follow' is True for a single ToolTip instance")
            bindings.append(
                Binding(self.widget, "<Motion>", self._update_tooltip_coords)
            )
        elif self.persist:
            bindings.append(
                Binding(self.widget, "<Motion>", self.on_leave)
            )    
